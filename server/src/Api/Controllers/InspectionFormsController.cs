...
public sealed class InspectionFormsController : ControllerBase
{
    ...
    [ProducesResponseType(typeof(List<GetAllInspectionFormItemDTO>), StatusCodes.Status200OK)]
    [ProducesResponseType(StatusCodes.Status404NotFound)]
    [ProducesResponseType(StatusCodes.Status500InternalServerError)]
    public async Task<ActionResult<List<GetAllInspectionFormItemDTO>>> GetAllInspectionFormItems(
        int id,
        CancellationToken cancellationToken)
    {
        try
        {
            ...
            var query = new GetAllInspectionFormItemQuery(id);
